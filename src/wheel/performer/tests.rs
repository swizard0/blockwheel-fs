...
fn script_basic() {
    ...
    let script = vec![
        ...
        ScriptOp::ExpectInterpretTask {
            expect_offset: 24,
            expect_task_kind: task::TaskKind::MarkTombstone(task::MarkTombstone {
                block_id: block::Id::init(),
                context: task::MarkTombstoneContext::External("ctx06"),
            }),
        },
        ...
        ScriptOp::ExpectInterpretTask {
            expect_offset: 77,
            expect_task_kind: task::TaskKind::ReadBlock(task::ReadBlock {
                block_header: storage::BlockHeader {
                    block_id: block::Id::init().next(),
                    block_size: 13,
                    ..Default::default()
                },
                block_bytes: block::BytesMut::new(),
