...
enum ActorInnerState {
    Active(ActorInnerStateActive),
    Terminated,
}

struct ActorInnerStateActive {
    mailbox: Mailbox,
    suspended: Option<JobArgs>,
}
...
impl Actor {
    pub fn send<P>(&self, message: Message, thread_pool: &P) -> Result<(), Error> where P: edeltraud::ThreadPool<job::Job> {
        let maybe_suspended =
            match *self.inner.state.lock().unwrap() {
                ActorInnerState::Active(ref mut state) => {
                    state.mailbox.messages.push(message);
                    state.suspended.take()
                },
                ActorInnerState::Terminated =>
                    return Err(Error::ActorIsTerminated),
            };
        if let Some(job_args) = maybe_suspended {
            edeltraud::job(
                thread_pool,
                PerformerActorJob {
                    actor: self.clone(),
    ...
    fn recv(&self, job_args: JobArgs) -> Result<RecvOutcome, Error> {
        match *self.inner.state.lock().unwrap() {
            ActorInnerState::Active(ref mut state) =>
                match state.mailbox.messages.pop() {
                    Some(message) =>
                        Ok(RecvOutcome::MessageReady { message, job_args, }),
                    None => {
                        assert!(state.suspended.is_none());
                        state.suspended = Some(job_args);
...
pub struct PerformerActorJob {
    actor: Actor,
    job_args: JobArgs,
}

pub fn run_job<P>(actor_job: PerformerActorJob, thread_pool: &P) where P: edeltraud::ThreadPool<job::Job> {
    let PerformerActorJob {
        actor,
        job_args: JobArgs {
            env,
            kont,
